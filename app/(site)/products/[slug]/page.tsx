...
export default async function ProductPage({ params }: Props) {
  ...
  const collectionHref = product.collection
    ? resolveInternalHref({ _type: 'collection', slug: product.collection.slug })
    : null
          ...
          <div className="flex flex-col gap-6">
            ...
            {product.collection && (
              collectionHref !== '#' ? (
                ...
                <span className="text-sm text-stone-500">{product.collection.title}</span>
              )
            )}
            ...
            {product.description && product.description.length > 0 && (
              <PortableTextRenderer value={product.description} className="prose prose-stone" />
            )}

            {/* Materials */}
            {product.materials && product.materials.length > 0 && (
              <div>
                <h2 className="mb-1 text-sm font-medium uppercase tracking-wide text-stone-500">Materials</h2>
            ...
            {product.careInstructions && (
              <div>
                <h2 className="mb-1 text-sm font-medium uppercase tracking-wide text-stone-500">Care</h2>
            ...
            {product.dimensions && (
              <div>
                <h2 className="mb-2 text-sm font-medium uppercase tracking-wide text-stone-500">Dimensions</h2>
                <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                  {product.dimensions.height != null && (
                    <>
                      <dt className="text-stone-500">Height</dt>
                      <dd className="text-stone-700">{product.dimensions.height} {product.dimensions.unit}</dd>
                    </>
                  )}
                  {product.dimensions.width != null && (
                    <>
                      <dt className="text-stone-500">Width</dt>
                      <dd className="text-stone-700">{product.dimensions.width} {product.dimensions.unit}</dd>
                    </>
                  )}
                  {product.dimensions.depth != null && (
                    <>
                      <dt className="text-stone-500">Depth</dt>
                      <dd className="text-stone-700">{product.dimensions.depth} {product.dimensions.unit}</dd>
                    </>
                  )}
                  {product.dimensions.weight != null && (
                    <>
                      <dt className="text-stone-500">Weight</dt>
                      <dd className="text-stone-700">{product.dimensions.weight}</dd>
                    </>
                  )}
                </dl>
              </div>
